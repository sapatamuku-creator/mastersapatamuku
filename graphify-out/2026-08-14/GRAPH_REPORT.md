# Graph Report - mastersapatamuku  (2026-08-14)

## Corpus Check
- 113 files · ~810,293 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 775 nodes · 846 edges · 107 communities (84 shown, 23 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `95bc7b1c`
...
8. `Spec: Phase 2 — Verifikasi Akun Vendor & Promo Diskon Marketplace` - 12 edges
9. `🔐 Rencana Migrasi: Subdomain Resolver → Opsi C (Supabase Edge Function)` - 11 edges
10. `handler()` - 10 edges
...
Cohesion: 0.07
Nodes (29): C1. Hardcoded Supabase API Key (10+ file), C2. Password Plaintext di Database, C3. SECURITY DEFINER Function Tanpa Sanitization, C4. Broken RLS Logic di config_welcome, C5. Stored XSS via innerHTML (8+ file), C6. Privilege Escalation via URL Parameter, C7. Hardcoded Payment Gateway Key, CRITICAL — Temuan Detail (+21 more)
...
### Community 62 - "mp.js"
Cohesion: 0.16
Nodes (27): CATEGORY_UUID_MAP, DEFAULT_CATEGORIES, ensureCategoryInDB(), findVendorByEmail(), generateSlug(), genOtpCode(), getValidUuidCategory(), getVendorFromToken() (+19 more)
...
Cohesion: 0.12
Nodes (17): H10. Session di localStorage, H11. Password dikirim sebagai URL Query, H12. OTP di-generate di Client, H13. Anon Full CRUD di Beberapa Table, H14. Anon Full Akses sortir_events & sortir_selections, H15. Anon Bisa Inject Kick Signals, H16. package-lock.json di-ignore, H1. Demo Mode Tanpa Auth (+9 more)
...
Cohesion: 0.16
Nodes (19): CACHE_LONG, CACHE_PUBLIC, config, CORS, DEFAULT_CATEGORIES, enrichPromo(), generateSlug(), genOtpCode() (+11 more)
...
## Knowledge Gaps
- **404 isolated node(s):** `config`, `DEFAULT_CATEGORIES`, `CORS`, `CACHE_PUBLIC`, `CACHE_LONG` (+399 more)
...
- **Why does `Security Fix Tasks — SapaTamu.Ku` connect `Vercel API: Get Email Handler` to `HIGH TASKS (16)`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **Why does `SECURITY AUDIT REPORT — SapaTamu.Ku` connect `HIGH — Temuan Detail` to `LOW TASKS (14)`?**
