...
-- Helper function: validate ssid format (alphanumeric + underscores, 10-50 chars)
CREATE OR REPLACE FUNCTION public.is_valid_ssid(s text)
RETURNS boolean AS $$
BEGIN
  RETURN s IS NOT NULL AND length(s) >= 10 AND length(s) <= 50 AND s ~ '^[a-zA-Z0-9_-]+$';
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- 2. TABLE: tamu (row-level: ssid must match filter)
...
CREATE POLICY "Anon select tamu by ssid" ON tamu
  FOR SELECT TO anon USING (is_valid_ssid(ssid));
CREATE POLICY "Anon insert tamu" ON tamu
  FOR INSERT TO anon WITH CHECK (is_valid_ssid(ssid));
CREATE POLICY "Anon update tamu by ssid" ON tamu
  FOR UPDATE TO anon USING (is_valid_ssid(ssid))
  WITH CHECK (is_valid_ssid(ssid));
CREATE POLICY "Anon delete tamu by ssid" ON tamu
  FOR DELETE TO anon USING (is_valid_ssid(ssid));
