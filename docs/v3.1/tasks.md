...
- [x] **T1.26 S — `checkin.html` & `onsite.html` App-Maximized Viewport Mode (Anti-Black Screen Tablet/Mobile)**
  - Menerapkan mode Layar Maksimal berbasis CSS `100dvh` untuk Mobile & Tablet (`< 1024px`) menggantikan native `requestFullscreen()` yang memicu bug SurfaceView Android di `checkin.html` dan `onsite.html`.
