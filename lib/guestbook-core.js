...
// --- Fetch tamu (chunked-ready, tapi T2.1 masih limit 1000 single shot agar kompatibel) ---
export async function fetchTamu(ssId, opts={}){
  ...
  // keep legacy select * behaviour via no select param, but request limit for safety
  const url = `${SB_URL}/rest/v1/tamu?ssid=eq.${ssId}&order=row.desc&limit=${limit}`;
