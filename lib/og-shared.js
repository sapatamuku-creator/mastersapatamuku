...
export async function resolveProductById(id) {
  if (!id) return null;
  const pRes = await sbFetchPublic(`/mp_products?id=eq.${encodeURIComponent(id)}&status=eq.publish&select=*&limit=1`);
...
export function buildProductOgMeta(product, vendor) {
  ...
  // Phase 2: promo aktif ⇔ harga diskon valid dan sekarang dalam periode
  const promoActive = price > 0
    && Number(product.discount_price) > 0
    && Number(product.discount_price) < price
    && !!product.promo_start_at && !!product.promo_end_at
    && Date.now() >= new Date(product.promo_start_at).getTime()
    && Date.now() <= new Date(product.promo_end_at).getTime();

  let priceTxt = price > 0 ? `Rp ${price.toLocaleString('id-ID')}` : 'harga menarik';
  if (promoActive) {
    priceTxt = `promo Rp ${Number(product.discount_price).toLocaleString('id-ID')} (normal Rp ${price.toLocaleString('id-ID')})`;
  }
  ...
  // Cover data: URI tidak bisa di-fetch crawler → sajikan lewat endpoint edge
  // yang mendecode base64 menjadi bytes gambar asli.
  let image = normaliseImageUrl(product.cover_image_url);
  if (!image && /^data:image\//i.test(String(product.cover_image_url || '')) && product.id) {
    image = `https://sapatamu.id/api/mp/product-cover?id=${encodeURIComponent(product.id)}`;
  }
  image = image
...
// ── SapaTamu Sortir: Dynamic Event OG Meta ──

export async function resolveSortirEventBySlug(slug) {
  if (!slug) return null;
  const cols = 'id,event_name,event_slug,quota_limit,drive_folder_id,created_at';
  const res = await sbFetchPublic(`/sortir_events?event_slug=eq.${encodeURIComponent(slug)}&select=${cols}&limit=1`);
  const rows = await res.json();
  const event = (Array.isArray(rows) && rows[0]) ? rows[0] : null;
  if (!event) return null;

  // Resolusi foto pertama dari Google Drive untuk cover dinamis
  if (event.drive_folder_id) {
    try {
      const apiKey = process.env.GOOGLE_API_KEY;
      if (apiKey) {
        const q = `'${event.drive_folder_id}' in parents and trashed = false`;
        const driveUrl = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&key=${apiKey}&fields=files(id,mimeType,name)&pageSize=10&orderBy=name`;
        const driveRes = await fetch(driveUrl);
        if (driveRes.ok) {
          const data = await driveRes.json();
          const files = data.files || [];
          
          let imgFile = files.find(f => f.mimeType && f.mimeType.startsWith('image/'));
          
          // Jika belum ada file foto langsung di root, cari 1 file di dalam subfolder
          if (!imgFile) {
            const subfolder = files.find(f => f.mimeType === 'application/vnd.google-apps.folder');
            if (subfolder) {
              const subQ = `'${subfolder.id}' in parents and trashed = false`;
              const subRes = await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(subQ)}&key=${apiKey}&fields=files(id,mimeType,name)&pageSize=5&orderBy=name`);
              if (subRes.ok) {
                const subData = await subRes.json();
                imgFile = (subData.files || []).find(f => f.mimeType && f.mimeType.startsWith('image/'));
              }
            }
          }

          if (imgFile && imgFile.id) {
            event.cover_image_url = `https://sapatamu.id/api/sortir-drive-img?id=${imgFile.id}&sz=w1200`;
          }
        }
      }
    } catch (e) {
      console.error('[Sortir OG Drive error]:', e);
    }
  }
...
export function buildSortirOgMeta(event) {
  const eventName = cleanMeta(event.event_name) || 'Galeri Culling & Seleksi Foto';
  const quota = event.quota_limit || 0;
  
  const title = eventName;
  let description = `Silakan pilih hingga maksimal ${quota > 0 ? quota + ' ' : ''}foto terbaik Anda. Pilihan foto tersimpan secara otomatis.`;
  description = description.slice(0, 160);

  const image = event.cover_image_url || 'https://sapatamu.id/assets/icon-sortir-512.png';
